...
# openalpr не заводится для произвольного шаблона
...
# you need to have installed openALPR https://github.com/openalpr/openalpr/releases
# Добавьте путь к openalpr в PATH или используйте переменные, как ниже
...
def recognize_with_openalpr(image):
    bin_dir = ".\openalpr_64" # поправить на корректный
    data_dir = r"D.\openalpr_64\runtime_data" # поправить на корректный
    conf_file = os.path.join(data_dir, "lit.conf")
    os.environ['PATH'] = bin_dir + ";" + os.environ['PATH']
    try:
        from ctypes import cdll
        cdll.LoadLibrary(os.path.join(bin_dir, "openalpr.dll"))
        print("DLL успешно загружена!")
    except Exception as e:
        print("DLL load error:", e)
    alpr = Alpr('rus', conf_file, bin_dir)
    if not alpr.is_loaded():
        print("Ошибка загрузки OpenALPR!")
        sys.exit(1)
...
def process_image_from_memory(
    ...
    while len(codes) < 3 and size > 30 and attempts < max_attempts:
        ...
        recognized_text = recognize_with_openalpr(resized)
